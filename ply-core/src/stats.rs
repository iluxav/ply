...
pub fn collect(filter: Option<&str>, sample_ms: u64) -> Result<Vec<InstanceStats>> {
    ...
    for (instance, first_usec) in instances.iter().zip(first) {
        let cg = cgroup_dir(instance);
        let use_cgroup = cg.exists();
        ...
        let (mem_current, mem_peak, mem_max, throttled, pids_current) = if use_cgroup {
            (
...
fn cgroup_dir(instance: &InstanceState) -> PathBuf {
    PathBuf::from(format!(
        "/sys/fs/cgroup/ply-{}.{}",
