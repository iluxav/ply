...
cp "$HERE/api/server" "$HERE/ply/autoapi/server"
cp "$HERE/api/server" "$HERE/ply/autoapi-metric/server"
cp "$HERE/api/server" "$HERE/docker/server"

echo "==> ply images"
(cd "$HERE/ply/api" && "$PLY" build .)
(cd "$HERE/ply/pgdb" && "$PLY" build .)
(cd "$HERE/ply/autoapi" && "$PLY" build .)
(cd "$HERE/ply/autoapi-metric" && "$PLY" build .)
...
echo "==> done"
ls -la "$HERE"/ply/api/*.img "$HERE"/ply/pgdb/*.img "$HERE"/ply/autoapi/*.img
